...
class CakeBosskeyTest {
    @Test
    fun `THECAKEISALIE shows the security-feed monologue`() = runTest {
    ...
    @Test
    fun `any accepted key toggles from the cake monologue to the bosskey spreadsheet`() = runTest {
        val engine = loginToShell()
        submit(engine, "THECAKEISALIE")
    ...
    @Test
    fun `toggling again from bosskey returns to the cake monologue`() = runTest {
        val engine = loginToShell()
        submit(engine, "THECAKEISALIE")
        val cakeScreen = engine.liveLine.value
    ...
    @Test
    fun `the toggle keeps working indefinitely`() = runTest {
        val engine = loginToShell()
        submit(engine, "THECAKEISALIE")
        val cakeScreen = engine.liveLine.value
    ...
    @Test
    fun `an unaccepted key does not toggle the cake-bosskey loop`() = runTest {
        val engine = loginToShell()
        submit(engine, "THECAKEISALIE")
    ...
    @Test
    fun `the cake easter egg never sets exitRequested`() = runTest {
        val engine = loginToShell()
        submit(engine, "THECAKEISALIE")
        pressKey(engine, "X")
        pressKey(engine, "Y")
