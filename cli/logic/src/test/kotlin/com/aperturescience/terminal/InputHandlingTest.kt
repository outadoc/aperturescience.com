...
class InputHandlingTest {
    @Test
    fun `input is swallowed entirely while the engine is locked mid-animation`() = runTest {
    ...
    @Test
    fun `digits letters space and question mark are all accepted`() = runTest {
    ...
    @Test
    fun `lowercase letters are uppercased as they are typed`() = runTest {
        val engine = bootAndSettle(TerminalEngine())
    ...
    @Test
    fun `punctuation other than question mark is rejected outright`() = runTest {
        val engine = bootAndSettle(TerminalEngine())
        for (key in listOf(".", ",", "!", "@", "#", "-", "_", "/", "'", "\"")) {
            engine.onKeyEvent(key)
        }
    ...
    @Test
    fun `special key names other than Enter Backspace PageUp PageDown are ignored as text`() = runTest {
        val engine = bootAndSettle(TerminalEngine())
    ...
    @Test
    fun `Backspace removes the last typed character`() = runTest {
        val engine = bootAndSettle(TerminalEngine())
    ...
    @Test
    fun `Backspace on empty input does nothing`() = runTest {
        val engine = bootAndSettle(TerminalEngine())
        pressKey(engine, "Backspace")
        assertEquals("> ", engine.liveLine.value)
    }
    ...
    @Test
    fun `input is capped at sixty-five characters`() = runTest {
        val engine = bootAndSettle(TerminalEngine())
    ...
    @Test
    fun `onKeyEvent always reports the key as handled`() = runTest {
        val engine = bootAndSettle(TerminalEngine())
