...
class NotesExeTest {
    ...
    @Test
    fun `opening NOTES shows the first history page`() = runTest {
    ...
    @Test
    fun `Enter pages through all four history entries`() = runTest {
        val engine = loginAsAdmin()
        submit(engine, "NOTES")
    ...
    @Test
    fun `any accepted key advances a page, not only Enter`() = runTest {
        val engine = loginAsAdmin()
        submit(engine, "NOTES")
    ...
    @Test
    fun `an unaccepted key does nothing while reading NOTES-EXE`() = runTest {
        val engine = loginAsAdmin()
        submit(engine, "NOTES")
    ...
    @Test
    fun `paging past the last history entry returns to the admin shell`() = runTest {
        val engine = loginAsAdmin()
        submit(engine, "NOTES")
